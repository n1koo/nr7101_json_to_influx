...
for name in nr7101_USERNAME nr7101_PASSWORD INFLUX_DB_HOST INFLUX_DB_DATABASE INFLUX_DB_USER INFLUX_DB_PASSWORD COOKIEPATH; do
    if [[ -z "${!name}" ]]; then
...
for ISP_NAME in telia elisa tele2; do
    ...
    nr7101-tool --cookie $COOKIEPATH/$ISP_NAME.cookie https://$nr7101_IP $nr7101_USERNAME $nr7101_PASSWORD >$ISP_NAME.json
